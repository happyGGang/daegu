...
$(document).ready(function() {
	...
	<%--1차 카테고리 변경--%>
	<%-- $('#archiveListForm select#large_code').on('change', function() {
		var largeCategoryCode = $(this).val();
		if ( $(this).val() != '--' ) {
			$.get('/${homepage.context_path}/module/archive/archiveCategory/getMidCategoryList.do?large_code=' + $(this).val(), function(data) {
				$('#archiveListForm select#mid_code option').remove();
				if ( data.length > 0 ) {
					var midCategoryCode = '--';
					$.each(data, function(i, v) {
						...
						$('#archiveListForm select#mid_code').append('<option value="' + v.mid_code + '">' + v.code_name + '</option>');
					});
					//2차 타케고리도변경(3차 카테고리 새로가져오기)
					if ( midCategoryCode != '--' ) {
						$.get('/${homepage.context_path}/module/archive/archiveCategory/getSmallCategoryList.do?large_code=' + largeCategoryCode + '&mid_code=' + midCategoryCode, function(data) {
							$('#archiveListForm select#small_code option').remove();
							if ( data.length > 0 ) {
								$('#archiveListForm select#small_code').append('<option value="--">전체</option>');
								$.each(data, function(i, v) {
									$('#archiveListForm select#small_code').append('<option value="' + v.small_code + '">' + v.code_name + '</option>');
								});
							}
							else {
								$('#archiveListForm select#small_code').append('<option value="--">전체</option>');
					...
					$('#archiveListForm select#mid_code').append('<option value="--">전체</option>');
					$('#archiveListForm select#small_code option').remove();
					$('#archiveListForm select#small_code').append('<option value="--">전체</option>');
				}
	...
	2차 카테고리 변경
	$('#archiveListForm select#mid_code').on('change', function() {
		var largeCategoryCode = $('#archiveListForm select#large_code').val();
		if ( $(this).val() != '--' ) {
			$.get('/${homepage.context_path}/module/archive/archiveCategory/getSmallCategoryList.do?large_code=' + largeCategoryCode + '&mid_code=' + $(this).val(), function(data) {
				$('#archiveListForm select#small_code option').remove();
				if ( data.length > 0 ) {
					$('#archiveListForm select#small_code').append('<option value="--">전체</option>');
					$.each(data, function(i, v) {
		...
		<li class="elib">
			...
			<div class="list-body">
				<div class="flexbox">
					...
					<div class="info">
						<p>${fn:escapeXml(i.large_code_name)}</p>
