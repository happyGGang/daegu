	...
	<div class="sview">
		...
		<c:if test="${detail.MANAGE_CODE eq 'BW'}">
			<p style="color:#ff0000;font-weight:bold;text-align:center;">
				* 도원도서관 장서점검으로 인한 상호대차 및 무인예약 신청 중지(9/23~10/06)를 안내드리오니, 많은 양해 부탁드립니다.
			</p>
		</c:if>
			...
			<c:choose>
				<c:when test="${homepage.context_path eq 'bukgs' || homepage.context_path eq 'bukdh' || homepage.context_path eq 'buktj'}">
					<c:choose>
						...
						<c:when test="${detail.MANAGE_CODE eq 'BA'  || detail.MANAGE_CODE eq 'BB' || detail.MANAGE_CODE eq 'BC' || detail.MANAGE_CODE eq 'GN' || detail.MANAGE_CODE eq 'GP' || detail.MANAGE_CODE eq 'HB' || detail.MANAGE_CODE eq 'HD' || detail.MANAGE_CODE eq 'HE' || detail.MANAGE_CODE eq 'GL'}">
							<c:choose>
				...
				<c:when test="${homepage.context_path eq 'dalseolib'}">
					...
					<c:choose>
						...
						<c:when test="${detail.MANAGE_CODE eq 'BU'||detail.MANAGE_CODE eq 'BV' ||detail.MANAGE_CODE eq 'BX' ||detail.MANAGE_CODE eq 'BY' ||detail.MANAGE_CODE eq 'BZ' ||detail.MANAGE_CODE eq 'FA' ||detail.MANAGE_CODE eq 'FB' ||detail.MANAGE_CODE eq 'FX' ||detail.MANAGE_CODE eq 'FC' ||detail.MANAGE_CODE eq 'GK'}">
							<c:choose>
								<c:when test="${detail.KBILL_LILL_YN eq 'O'}">
									<a href="" class="btn btn3 sangho"><span>상호대차 신청</span></a>
								</c:when>
...
<!--
			...
			<c:choose>
				...
				<c:when test="${homepage.context_path eq 'dalseolib'}">
					<c:if test="${detail.MANAGE_CODE eq 'BU'||detail.MANAGE_CODE eq 'BV' || detail.MANAGE_CODE eq 'BY' || detail.MANAGE_CODE eq 'BX' || detail.MANAGE_CODE eq 'BZ'}">
