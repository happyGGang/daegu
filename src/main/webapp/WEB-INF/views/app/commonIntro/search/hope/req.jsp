...
<c:choose>
	...
	$(function() {
		$('#save-btn').on('click', function(e) {
			...
			<c:if test="${homepage.context_path eq 'bukgs' or homepage.context_path eq 'bukdh' or homepage.context_path eq 'buktj' or homepage.context_path eq 'gw' or homepage.context_path eq 'donggu'}">
...
<c:when test="${homepage.context_path eq 'bukgs' || homepage.context_path eq 'buktj' || homepage.context_path eq 'bukdh'}">
	<div style='border:1px solid #ddd;box-sizing:border-box;border-radius:3px;padding:18px;margin-bottom:15px;'>
		<ul>
			...
			<!--<li><p style="color:red;">※ 대구 시 도서관 희망도서 검색 시스템 업데이트로 인해 현재 검색창 사용이 불가합니다. <br>당분간 희망도서 신청 시 검색창 하단에 도서제목 및 기타 정보를 기입하여 신청바랍니다</li>-->
		</ul>
	</div>
</c:when>
...
<%--				<form:option value="GJ">태전1동 작은도서관</form:option> 	--%>
				...
				<select id="manageCode" name="manageCode" class="new_select_box">
					...
					* 달서영어도서관 : 영어원서(서양서) 및 영어학습 관련(국내서)만 신청가능합니다.<br>
					...
					<!-- * 도원도서관, 달서가족문화도서관, 본리도서관 희망도서 25. 6. 01. 오전 10시 재개 예정<br> -->
				...
				<c:when test="${homepagePath eq 'gw'}">
				<form:select path="manageCode" cssClass="new_select_box">
		...
		<tr>
			<c:choose>
				<c:when test="${homepage.context_path eq 'donggu' or homepage.context_path eq 'bukgs' or homepage.context_path eq 'bukdh' or homepage.context_path eq 'buktj' or homepage.context_path eq 'gw'}">
					<th>ISBN <em><font color="red">(*)</font></em></th>
				</c:when>
				<c:otherwise>
