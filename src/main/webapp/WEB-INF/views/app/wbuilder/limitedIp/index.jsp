...
<form:form id="limitedIp_index" modelAttribute="limitedIp" action="save.do" method="post" onsubmit="return false;">
<form:hidden id="editMode_index" path="editMode"/>
<form:hidden id="limitedIp_idx" path="limitedIp_idx"/>
</form:form>
<div class="infodesk">
	검색 결과 : ${fn:length(limitedIpList)}건
	<div class="button btn-group inline">
		<c:if test="${authC}">
			<a href="" class="btn btn5 left" id="dialog-add"><i class="fa fa-plus"></i><span>접근불가능IP 추가</span></a>
		</c:if>
	</div>
</div>
<div class="table-wrap">
	<table class="type1 center">
		...
		<c:forEach var="i" varStatus="status" items="${limitedIpList}">
			<tr>
				...
				<td>
					<c:if test="${authU}">
						<a href="" class="btn" id="dialog-modify" keyValue="${i.limitedIp_idx}">수정</a>
					</c:if>
					<c:if test="${authD}">
						<a href="" class="btn" id="delete" keyValue="${i.limitedIp_idx}">삭제</a>
					</c:if>
				</td>
			</tr>
		</c:forEach>
...
<div id="dialog-1" class="dialog-common" title="접근불가능 IP등록/수정">
</div>