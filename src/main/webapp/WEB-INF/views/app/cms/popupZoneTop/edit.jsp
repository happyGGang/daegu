		...
		buttons: [
			{
				...
				click: function() {
					...
					$('#popupZoneTop').ajaxSubmit(option);
				}
			},{
				text: "취소",
				"class": 'btn',
...
<form:form modelAttribute="popupZoneTop" action="save.do" method="POST" onsubmit="return false;" enctype="multipart/form-data">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<c:choose>
					<c:when test="${popupZoneTop.homepage_id eq 'h1' }"><!-- 228기념학생도서관 -->
					<div class="ui-state-highlight">
						<em>* 팝업존 최적 이미지 사이즈는 가로 413 X 세로 325 입니다(픽셀단위)</em>
					</div>
					</c:when>
					<c:when test="${popupZoneTop.homepage_id eq 'h2' }"><!-- 228민주운동기념회관 -->
					<div class="ui-state-highlight">
						<em>* 팝업존 최적 이미지 사이즈는 가로 413 X 세로 430 입니다(픽셀단위)</em>
					</div>
					</c:when>
					<c:when test="${popupZoneTop.homepage_id eq 'h3' }"><!-- 남부 -->
					<div class="ui-state-highlight">
						<em>* 팝업존 최적 이미지 사이즈는 가로 410 X 세로 442 입니다(픽셀단위)</em>
					</div>
					</c:when>
					<c:when test="${popupZoneTop.homepage_id eq 'h4' }"><!-- 달성 -->
					<div class="ui-state-highlight">
						<em>* 팝업존 최적 이미지 사이즈는 가로 410 X 세로 430 입니다(픽셀단위)</em>
					</div>
					</c:when>
					<c:when test="${popupZoneTop.homepage_id eq 'h5' }"><!-- 동부 -->
					<div class="ui-state-highlight">
						<em>* 팝업존 최적 이미지 사이즈는 가로 600 X 세로 220 입니다(픽셀단위)</em>
					</div>
					</c:when>
					<c:when test="${popupZoneTop.homepage_id eq 'h6' }"><!-- 두류 -->
					<div class="ui-state-highlight">
						<em>* 팝업존 최적 이미지 사이즈는 가로 413 X 세로 328 입니다(픽셀단위)</em>
					</div>
					</c:when>
					<c:when test="${popupZoneTop.homepage_id eq 'h7' }"><!-- 북부 -->
					<div class="ui-state-highlight">
						<em>* 팝업존 최적 이미지 사이즈는 가로 413 X 세로 328 입니다(픽셀단위)</em>
					</div>
					</c:when>
					...
					<c:when test="${popupZoneTop.homepage_id eq 'h10' }"><!-- 중앙 -->
					<div class="ui-state-highlight">
		...
		<c:if test="${popupZoneTop.editMode eq 'MODIFY'}">
		<tr>
			<th>현재 이미지</th>
			<td>
				<img id="currentImg" name="currentImg" src="/data/popupZoneTop/${popupZoneTop.homepage_id}/${popupZoneTop.server_file_name}" alt="${popupZoneTop.server_file_name}"/>
			</td>
		</tr>
		</c:if>
		<tr>
