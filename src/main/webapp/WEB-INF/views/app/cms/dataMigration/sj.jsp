...
$(function(){
	$('button').click(function(){
		...
		if (isBook) {
			$('form#dm').attr('action', 'savesj2.do');
		} else if (isMovie) {
			$('form#dm').attr('action', 'savesj3.do');
		} else {
			$('form#dm').attr('action', 'savesj.do');
		}
		$('form#dm').attr('action', $('input#action').val());
...
<input type="text" id="action" value="savesj.do">
<form id="dm" name="dm" action="savesj.do" method="post" onsubmit="return false;">
<table class="type2">
				<colgroup>
					<col width="150"/>
					<col width="300"/>
