...
$(function(){
	...
	$('a#dialog-add').on('click', function(e) {
		$('#dialog-1').load('edit.do?editMode=ADD&homepage_id=${bookOfFamous.homepage_id}', function( response, status, xhr ) {
	...
	$('a.dialog-modify').on('click', function(e) {
		$('#dialog-1').load('edit.do?editMode=MODIFY&homepage_id=${bookOfFamous.homepage_id}&book_famous_idx=' + $(this).attr('keyValue'), function( response, status, xhr ) {
			$('#dialog-1').dialog('open');
		});
	...
	$('a.delete').on('click', function(e) {
		e.preventDefault();
		if (confirm('삭제하시겠습니까?')) {
			$('input#book_famous_idx_1').val($(this).attr('keyValue'));
...
<form:hidden id="book_famous_idx_1" path="book_famous_idx"/>
	...
	<table class="type1 center">
		...
		<c:forEach var="i" varStatus="status" items="${boyList}">
			<tr>
				...
				<td width="120">
					<c:if test="${authU}">
						<a href="#" class="btn dialog-modify" keyValue="${i.book_famous_idx}">수정</a>
					</c:if>
					<c:if test="${authD}">
						<a href="#" class="btn delete" keyValue="${i.book_famous_idx}">삭제</a>
					</c:if>
				</td>
			</tr>
		</c:forEach>
