...
$(function() {
	...
	$('a.dialog-edit').on('click', function(e) {
		e.preventDefault();
		$('#dialog-2').load('loanEdit.do?editMode=MODIFY&think_pocket_package_loan_idx='+$(this).attr('keyValue'), function(response, status, xhr) {
...
a.cancel-btn {border: 1px solid #787b80;color: #787b80;}
</style>
		...
		<tbody>
			<c:forEach var="i" varStatus="status" items="${loanList}">
				<tr>
					...
					<td class="left">
						<c:choose>
							<c:when test="${i.grade eq 1}">
								유아
							</c:when>
							<c:when test="${i.grade eq 2}">
								초등
							</c:when>
						</c:choose>
					</td>
					<td class="left">
						<a href="#" class="dialog-edit" keyValue="${i.think_pocket_package_loan_idx}">
							${i.think_pocket_package_subject}
							<br/>
