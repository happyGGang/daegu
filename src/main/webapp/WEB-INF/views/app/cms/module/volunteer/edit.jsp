...
<form:form modelAttribute="volunteer" id="volunteer_edit" action="save.do" method="post" onsubmit="return false;">
<form:hidden path="editMode"/>
<form:hidden path="plan_date" value="${fn:substring(calendarManage.start_date,0,7)}"/>
<form:hidden path="homepage_id"/>
<form:hidden path="volunteer_idx"/>
...
<table class="type2">
	...
	<tbody>
		<tr>
			<th>신청 종류(<span style="color: red; font-weight: bold;">*</span>)</th>
			<td>
				<c:choose>
					<c:when test="${volunteer.editMode eq 'ADD'}">
					...
					<c:otherwise>
						<c:forEach var="i" items="${dateTypeList}">
							<c:if test="${i.code_id eq volunteer.date_type}">${i.code_name}</c:if>
						</c:forEach>
		...
		<tr>
			<th>대관일자(<span style="color: red; font-weight: bold;">*</span>)</th>
				<td>
					...
					<c:if test="${volunteer.editMode eq 'ADD'}">
		...
		<c:choose>
		<c:when test="${volunteer.homepage_id ne 'h50'}">
		<tr>
		...
		<tr>
			...
			<td>
				<form:select path="use_time">
					...
					<form:option value="pm" label="오후"></form:option>
				</form:select>					
			</td>
		</tr>
		</c:otherwise>
