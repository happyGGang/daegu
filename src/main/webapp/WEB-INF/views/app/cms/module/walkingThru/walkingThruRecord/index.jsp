...
$(function(){
	$('#all-check').on('click', function(e) {
		...
		if($(this).attr('keyValue') == 'N') {
			...
			$('.record_idx').prop('checked', true);
		} else {
			$(this).attr('keyValue', 'N');
			$('.record_idx').prop('checked', false);
		}
	});
	...
	}).datepicker('setDate', '${walkingThruRecord.start_date}');
	...
	}).datepicker('setDate', '${walkingThruRecord.end_date}');
...
<label class="blind">검색</label>
	검색 결과 : ${walkingThruRecordListCount}건
	<form:select path="rowCount" class="selectmenu" style="width:150px;">
		...
		<form:option value="${walkingThruRecordListCount}">전체 보기</form:option>
	</form:select>
	대출단계 : 
...
<table class="type1 center">
	...
	<c:if test="${fn:length(walkingThruRecordList) < 1}">
		<tr style="height:100%">
			<td colspan="11" style="background:#f8fafb;">워킹스루 도서대출 신청내역이 없습니다.</td>
		</tr>
	</c:if>
	<c:forEach var="i" varStatus="status" items="${walkingThruRecordList}">
		<tr>
			<td width="10"><form:checkbox path="request_number_arr" cssClass="record_idx" value="${i.request_number}"/></td>
...
<jsp:include page="/WEB-INF/views/app/cms/common/paging.jsp" flush="false">
	<jsp:param name="formId" value="#walkingThruRecord"/>
</jsp:include>
