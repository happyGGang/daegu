...
<script type="text/javascript">
	function goSearch() {
		$('#viewPage').val(1);
		doGetLoad('user/index.do', serializeCustom($('form#lectureInfo')));
	}
...
$(function() {
	...
	$(function() {
		$('input#request_start_date').datepicker({
			onClose: function(selectedDate) {
				$('input#request_end_date').val(selectedDate);
	...
	function resetForm() {
		$('#edu_school option:eq(0)').prop('selected', true);
		$('#edu_target option:eq(0)').prop('selected', true);
		$('#edu_category option:eq(0)').prop('selected', true);
		$('#search_edu_status option:eq(0)').prop('selected', true);
		$('#search_request_type option:eq(0)').prop('selected', true);
		$('#request_start_date').val('');
				...
				<c:forEach var="i" varStatus="status" items="${lectureInfoList}">
					...
					<li class="table">
						<div class="lecture">
							...
							<div class="bott_box">
								<p class="state_list" style="display: inline-block;">
									<c:choose>
										<c:when test="${i.lecture_status1 eq '모집예정'}">
											<span class="lec_state state1 tit">모집예정</span>
										</c:when>
										<c:when test="${i.lecture_status1 eq '모집중'}">
											<c:if test="${i.online_person_count <= i.online_request_count and i.wait_person_count <= i.wait_request_count}">
												<span class="lec_state state4 tit">모집마감</span>
											</c:if>
											<c:if test="${i.online_person_count > i.online_request_count or i.wait_person_count > i.wait_request_count}">
												<span class="lec_state state2 tit">모집중</span>
											</c:if>
										</c:when>
								...
								<c:if test="${i.lecture_status1 eq '모집중'}">
									<c:if test="${i.online_person_count <= i.online_request_count and i.wait_person_count <= i.wait_request_count}">
									...
									<c:if test="${i.online_person_count > i.online_request_count or i.wait_person_count > i.wait_request_count}">
										<div class="search btn-inline" style="display: inline-block; margin: 0; padding: 0;">
											<fieldset style="display: inline-block; margin: 0; padding: 0;">
												<button class="app_btn app_color3 check btn-inline" type="button" onclick="lectureRequest('${i.lecture_id}');" style="margin:0;">수강신청</button>
											</fieldset>
										</div>
									</c:if>
								</c:if>
								<%--<div id="request_layer_${i.lecture_id}" class="new_layer smlayer lay_middle" style="display:none;">
