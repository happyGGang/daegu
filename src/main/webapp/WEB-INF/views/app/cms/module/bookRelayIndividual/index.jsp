...
$(function() {
	...
	$('select#rowCount').on('change', function() {
		$('#viewPage').val(1);
		doGetLoad('index.do', $('form#bookRelayIndividual').serialize());
	});
	...
	$('a.view_btn').on('click', function(e){
		...
		doGetLoad('view.do', serializeCustom($('form#bookRelayIndividual')));
	});
	
	$('select.approval_status').on('change', function(e) {
		e.preventDefault();
		$('form#bookRelayIndividual').attr('action', 'statusChange.do');
		$('#individual_idx').val($(this).data('key'));
	...
	$('a.delete_btn').on('click', function(e) {
		...
		if (confirm('정말로 삭제하시겠습니까?\n\n삭제된 신청은 복구가 불가능합니다.')) {
			$('form#bookRelayIndividual').attr('action', 'delete.do');
			$('#individual_idx').val($(this).data('key'));
			$('#editMode').val('DELETE');
			doAjaxPost($('form#bookRelayIndividual'));
			location.reload();
	...
	$('button#search_btn').on('click', function(e){
		e.preventDefault();
		$('#viewPage').val(1);
		doGetLoad('index.do', serializeCustom($('form#bookRelayIndividual')));
	});

	$('a#excelDownload').on('click', function(e) {
		$('#bookRelayIndividual').attr('action', 'excelDownload.do').submit();
		$('#bookRelayIndividual').attr('action', 'save.do');
		e.preventDefault();
	});

	$('a#csvDownload').on('click', function(e) {
		e.preventDefault();
		$('#bookRelayIndividual').attr('action', 'csvDownload.do').submit();
	});
	
});
</script>

<form:form modelAttribute="bookRelayIndividual">
...
<form:hidden path="approval_status"/>
	...
	<div>
		<table class="type1 center">
			...
			<tbody>
				<c:forEach var="i" varStatus="status" items="${bookRelayIndividualList}">
					<tr>
						<td>${paging.listRowNum - status.index}</td>
						<td>
							<a href="#" class="view_btn" data-key="${i.individual_idx}">${i.user_name}</a>
						</td>
						<td>${i.user_phone}</td>
						<td>
							<c:choose>
						...
						<td>
							<select class="approval_status" data-key="${i.individual_idx}">
				...
				<c:if test="${fn:length(bookRelayIndividualList) < 1}">
					<tr>
		...
		<jsp:include page="/WEB-INF/views/app/cms/common/paging.jsp" flush="false">
			<jsp:param name="formId" value="#bookRelayIndividual"/>
		</jsp:include>
