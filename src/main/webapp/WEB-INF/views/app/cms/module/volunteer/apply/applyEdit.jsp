		...
		buttons: [
			{
				...
				click: function() {		
					if('${fn:length(applyList)}' > 0) {
						$('#apply_edit2').attr('action', '/cms/module/volunteer/apply/excelDownload.do').submit();
// 						$('#apply_edit2').attr('action', '/cms/module/volunteer/apply/save.do');	
					} else {
						alert('해당 내역이 없습니다.');	
					}
				}
	...
	$('a#apply-modify').on('click', function(event) {
		$('#dialog-2').load('/cms/module/volunteer/apply/edit.do?editMode=MODIFY&homepage_id=' + $('#homepage_id_1').val() + '&apply_idx=' + $(this).attr('keyValue') + '&use_time=' + $(this).attr('keyValue2'), function( response, status, xhr ) {
	...
	$('a#state-modify').on('click', function(event) {
		$('#dialog-4').load('/cms/module/volunteer/apply/stateEdit.do?editMode=STATEMODIFY&homepage_id=' + $('#homepage_id_1').val() + '&apply_idx=' + $(this).attr('keyValue'), function( response, status, xhr ) {
	...
	$('a#delete-btn').on('click', function(event) {
		if(confirm('해당 정보를 삭제 하시겠습니까?')) {
			...
			if(doAjaxPost($('#apply_edit2'))) {
				$('#dialog-3').load('/cms/module/volunteer/apply/applyEdit.do?editMode=VIEW&homepage_id=' + $('#homepage_id').val() + '&volunteer_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('plan_date'), function( response, status, xhr ) {
...
<form:form modelAttribute="volunteerApply" id="apply_edit2" action="/cms/module/volunteer/apply/save.do" method="post">
		...
		<tbody>
			<c:forEach var="i" varStatus="status" items="${applyList}">
				...
				<tr>
					...
					<c:if test="${volunteerApply.editMode ne 'VIEW' }">
						<td>
