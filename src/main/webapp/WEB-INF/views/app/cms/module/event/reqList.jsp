...
$(function() {
		...
		buttons: [
			{
				...
				click: function() {
					if(confirm('개인정보(등록ID, 신청자명, 반, 전화번호)를 삭제합니다.\n삭제된 데이터는 복구가 불가능합니다.\n\n계속 하시겠습니까?')) {
						if(doAjaxPost($('#deletePersonalDataForm'))) {
							$('#dialog-3').load('reqList.do?homepage_id=${event.homepage_id}&event_idx=${event.event_idx}', function( response, status, xhr ) {
				...
				click: function() {
					<c:choose>
					<c:when test="${event.select_cnt eq 0}">
					if(confirm('당첨자 추첨을 시작합니다.\n\n무작위 순서로 정답자 중에서 당첨자가 선정되고 한번 선정되면 되돌릴 수 없습니다. 또한 해당 이벤트의 문항 정보(보기 및 정답)를 수정할 수 없게 됩니다.\n\n계속 하시겠습니까?')) {
				...
				click: function() {
					<c:choose>
					<c:when test="${event.re_select_cnt eq 0}">
					if(confirm('당첨자 재추첨을 시작합니다.\n\n무작위 순서로 정답자 중에서 당첨자로 선정되지 않으신 분에 한해서 재 추첨을 시작합니다. 또한 해당 이벤트의 문항 정보(보기 및 정답)를 수정할 수 없게 됩니다.\n\n계속 하시겠습니까?')) {
				...
				"class": 'btn btn2',
				click: function() {		
					if('${fn:length(eventReqList)}' > 0) {
						$('#winnerForm input#rowCount').val('1000');
						$('#winnerForm').attr('action', '/cms/module/eventReq/excelDownload.do').submit();
				...
				click: function() {		
					if('${fn:length(eventReqList)}' > 0) {
						$('#winnerForm input#rowCount').val('1000');
						$('#winnerForm').attr('action', '/cms/module/eventReq/csvDownload.do').submit();
						$('#winnerForm').attr('action', 'saveWinner.do');	
					} else {
						alert('해당 내역이 없습니다.');
					}
	...
	$('select[name="winner_yn"]').change(function() {
		$('#winnerForm input#event_req_idx').val($(this).attr('keyValue'));
	...
	$('div#cms_paging_ajax a').on('click', function(e) {
		...
		var param = $('form#eventForm').serialize();
...
<form:form id="winnerForm" modelAttribute="eventReq" action="saveWinner.do">
	<form:hidden path="homepage_id"/>
	<form:hidden path="event_idx"/>
	<form:hidden path="event_req_idx"/>
	<form:hidden path="winner_yn"/>
	<form:hidden path="rowCount"/>
</form:form>

<form:form id="shuffleForm" modelAttribute="eventReq" action="shuffle.do">
	<form:hidden path="homepage_id" id="shuffleForm_homepage_id"/>
	<form:hidden path="event_idx" id="shuffleForm_event_idx"/>
	<form:hidden path="event_req_idx" id="shuffleForm_event_req_idx"/>
	<input type="hidden" name="max" id="shuffleForm_max" value="0">
</form:form>

<form:form id="reShuffleForm" modelAttribute="eventReq" action="reShuffle.do">
	<form:hidden path="homepage_id" id="shuffleForm_homepage_id"/>
	<form:hidden path="event_idx" id="shuffleForm_event_idx"/>
	<form:hidden path="event_req_idx" id="shuffleForm_event_req_idx"/>
...
<form:form id="deletePersonalDataForm" modelAttribute="eventReq" action="deletePersonalData.do">
	<form:hidden path="homepage_id" id="deletePersonalDataForm_homepage_id"/>
	<form:hidden path="event_idx" id="deletePersonalDataForm_event_idx"/>
</form:form>
...
[ 정답 : 
	<c:forEach items="${eventQuestionList}" var="oneQuestion" varStatus="questionStatus">
		${questionStatus.count} - ${oneQuestion.event_question_answer} <c:if test="${!questionStatus.last}">/</c:if>
	</c:forEach>
	]
</div>
<br/>
<form:form id="eventForm" modelAttribute="eventReq" action="reqList.do" >
	<form:hidden path="homepage_id"/>
	<form:hidden path="event_idx"/>	
	...
	<table class="type1 center">
		<colgroup>
       	...
       	<tbody>
       		<c:choose>
       			<c:when test="${fn:length(eventReqList) > 0}">
       				<c:forEach items="${eventReqList}" var="i">
			       		<tr>
				         	<td>${i.event_req_idx}</td>
				         	<td>${i.add_id}</td>
				         	<td>${i.name}</td>
							<td>${i.applicant_id}</td>
				         	<td class="left">
								<c:forTokens items="${i.event_answer}" delims="|" var="oneAnswer" varStatus="status">
				         			<span <c:if test="${fn:replace(fn:trim(eventQuestionList[status.index].event_question_answer), ' ', '') eq fn:replace(fn:trim(oneAnswer), ' ', '')}">style="background:#0f0"</c:if>>${status.count}번 답: ${oneAnswer}</span><br/>
				         		</c:forTokens>
			         		</td>
			         		<td>${i.school}</td>
				         	<td>${i.hak}</td>
				         	...
				         	<td>
				         		${i.winner_yn}
<%--
				         		<select class="selectmenu" name="winner_yn" keyValue="${i.event_req_idx}">
