...
$(function(){
	...
	var planDate = '${consultings.plan_date}'.split('-');
	var planDateMonth = '${consultings.plan_date}';
	...
	$('a#apply').on('click', function(event) {
		$('#dialog-2').load('/cms/module/consultings/apply/edit.do?editMode=ADD&homepage_id=' + $('#homepage_id_1').val() + '&consultings_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2'), function( response, status, xhr ) {
	...
	$('a#check_apply').on('click', function(event) {
		$('#dialog-3').load('/cms/module/consultings/apply/applyEdit.do?editMode=ADD&homepage_id=' + $('#homepage_id_1').val() + '&consultings_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2'), function( response, status, xhr ) {
	...
	$('a#excelDownloadDate').on('click', function(event) {
		...
		$('#dialog-5').load('/cms/module/consultings/apply/excelDownloadDate.do?homepage_id='+$('#homepage_id_1').val() + '&plan_year=' + $('#plan_year').val() + '&plan_month=' + $('#plan_month').val() , function( response, status, xhr ) {
	...
	$('a#totalExcelDownload').on('click', function(event) {
		...
		$('#plan_date').val("");
		$('#excursions').attr('action','/cms/module/consultings/apply/excelDownload.do').submit();
	});
	...
	$('a#csvDownload').on('click', function(e) {
		...
		$('#consultings').attr('action', '/cms/module/consultings/apply/csvDownloadMonth.do').submit();
	});
	...
	$('a#totalCsvDownload').on('click', function(e) {
		...
		$('#plan_date').val("");
		$('#consultings').attr('action', '/cms/module/consultings/apply/csvDownload.do').submit();
	});
	...
	$('a#before-btn').on('click', function(event) {
		...
		doGetLoad('index.do', serializeCustom($('#consultings')));
	...
	$('a#allChecked').on('click', function(e) {
		...
		$('input[name = consultings_idx_arr]').prop('checked', allChecked);
	});
	
	$('a#checkedDelete').on('click', function(e) {	
		e.preventDefault();
		if ($('input[name=consultings_idx_arr]:checked').length < 1) {
			alert('선택된 일자가 없습니다.');
			return false;
		}
		if(confirm('선택된 상담을 삭제 하시겠습니까?\n삭제시 신청자 정보등 모든 관련된 정보도 같이 삭제되며 복구 불가능합니다.')){
			$('form#consultings').attr('action','save.do');
			$('form#consultings').attr('method','post');
			$('form#consultings input#editMode').val('BATCHDELETE');
			if (doAjaxPost($('form#consultings'))) {
				location.reload();
			}
		}
	});
...
<form:form modelAttribute="consultings" action="" id="consultings">
				...
				<tr>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<c:set var="plan_date" value="${consultings.plan_date}-${fn:length(i.sun) < 2? '0' : ''}${i.sun}" />
							<tag:consultings plan_date="${plan_date}" consultingsList="${consultingsList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
					<c:choose>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<c:set var="plan_date" value="${consultings.plan_date}-${fn:length(i.mon) < 2? '0' : ''}${i.mon}" />
							<tag:consultings plan_date="${plan_date}" consultingsList="${consultingsList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<c:set var="plan_date" value="${consultings.plan_date}-${fn:length(i.tue) < 2? '0' : ''}${i.tue}" />
							<tag:consultings plan_date="${plan_date}" consultingsList="${consultingsList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<c:set var="plan_date" value="${consultings.plan_date}-${fn:length(i.wed) < 2? '0' : ''}${i.wed}" />
							<tag:consultings plan_date="${plan_date}" consultingsList="${consultingsList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<c:set var="plan_date" value="${consultings.plan_date}-${fn:length(i.thu) < 2? '0' : ''}${i.thu}" />
							<tag:consultings plan_date="${plan_date}" consultingsList="${consultingsList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<c:set var="plan_date" value="${consultings.plan_date}-${fn:length(i.fri) < 2? '0' : ''}${i.fri}" />
							<tag:consultings plan_date="${plan_date}" consultingsList="${consultingsList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<c:set var="plan_date" value="${consultings.plan_date}-${fn:length(i.sat) < 2? '0' : ''}${i.sat}" />
							<tag:consultings plan_date="${plan_date}" consultingsList="${consultingsList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
...
<div id="dialog-1" class="dialog-common" title="상담등록">
</div>
<div id="dialog-2" class="dialog-common" title="상담신청">
</div>
