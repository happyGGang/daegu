...
$(function(){
	...
	$('button#search_btn').on('click', function(e) {
		e.preventDefault();
		$('#viewPage').val(1);
		doGetLoad('index.do', $('form#walkingThruBlackList').serialize());
	});
	...
	}).datepicker('setDate', '${walkingThruBlackList.start_date}');
	...
	}).datepicker('setDate', '${walkingThruBlackList.end_date}');
	...
	$('a#excelDownload').on('click', function(e) {
			$('#walkingThruBlackList').attr('action', 'excelDownload.do').submit();
			$('#walkingThruBlackList').attr('action', 'save.do')
		e.preventDefault();
	});
...
<form:form id="walkingThruBlackList" modelAttribute="walkingThruBlackList" method="POST" action="save.do">
<form:hidden id="homepage_id" path="homepage_id"/>

<div class="search">
<label class="blind">검색</label>
	검색 결과 : ${walkingThruBlackListCount}건
	<form:select path="rowCount" class="selectmenu" style="width:150px;">
		...
		<form:option value="${walkingThruBlackListCount}">전체 보기</form:option>
	</form:select>
	패널티 사유 :
...
<table class="type1 center">
	...
	<tbody>
	<c:if test="${fn:length(walkingThruBlackListList) < 1}">
		<tr style="height:100%">
			<td colspan="8" style="background:#f8fafb;">블랙리스트 내역이 없습니다.</td>
		</tr>
	</c:if>
	<c:forEach var="i" varStatus="status" items="${walkingThruBlackListList}">
		<tr>
...
<jsp:include page="/WEB-INF/views/app/cms/common/paging.jsp" flush="false">
	<jsp:param name="formId" value="#walkingThruBlackList"/>
</jsp:include>
