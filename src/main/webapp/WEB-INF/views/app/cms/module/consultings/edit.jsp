		...
		buttons: [
			{
				...
				"class": 'btn btn1',
				click: function() {
					if(doAjaxPost($('#consultings_edit'))) {
						$(this).dialog('destroy');
						location.reload();
					}
				}
...
<form:form modelAttribute="consultings" id="consultings_edit" action="save.do" method="post" onsubmit="return false;">
<form:hidden path="editMode"/>
<form:hidden path="plan_date" value="${fn:substring(calendarManage.start_date,0,7)}"/>
<form:hidden path="homepage_id"/>
<form:hidden path="consultings_idx"/>
...
<table class="type2">
	...
	<tbody>
		<tr>
			<th>상담 종류(<span style="color: red; font-weight: bold;">*</span>)</th>
			<td>
				<c:choose>
					<c:when test="${consultings.editMode eq 'ADD'}">
					...
					<c:otherwise>
						<c:forEach var="i" items="${dateTypeList}">
							<c:if test="${i.code_id eq consultings.date_type}">${i.code_name}</c:if>
						</c:forEach>
		...
		<tr>
			<th>상담일자(<span style="color: red; font-weight: bold;">*</span>)</th>
				<td>
					...
					<c:if test="${consultings.editMode eq 'ADD'}">
		...
		<tr>
			<th>상담시간(<span style="color: red; font-weight: bold;">*</span>)</th>
			<td>
		...
		<tr>
			<th>상담신청기간(<span style="color: red; font-weight: bold;">*</span>)</th>
			<td>
