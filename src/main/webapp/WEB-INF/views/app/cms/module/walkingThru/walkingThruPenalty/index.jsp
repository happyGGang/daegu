...
<form:form modelAttribute="walkingThruPenalty" id="PenaltySettingForm" method="POST" action="save.do">
...
<form:hidden id="penalty_idx_1" path="penalty_idx"/>
	...
	<table class="type1 center">
		...
		<tbody>
			<c:forEach var="i" varStatus="status" items="${walkingThruPenaltyList}">
				<tr>
					<td>${walkingThruPenalty.listRowNum - status.index}</td>
					<td>${i.start_date}</td>
					<td>${i.end_date}</td>
			...
			<c:if test="${fn:length(walkingThruPenaltyList) < 1}">
				<tr style="height:100%">
