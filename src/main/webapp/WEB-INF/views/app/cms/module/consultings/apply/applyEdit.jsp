		...
		buttons: [
			{
				...
				click: function() {		
					if('${fn:length(applyList)}' > 0) {
						$('#apply_edit2').attr('action', '/cms/module/consultings/apply/excelDownload.do').submit();
// 						$('#apply_edit2').attr('action', '/cms/module/excursions/apply/save.do');	
	...
	$('a#apply-modify').on('click', function(event) {
		$('#dialog-2').load('/cms/module/consultings/apply/edit.do?editMode=MODIFY&homepage_id=' + $('#homepage_id_1').val() + '&apply_idx=' + $(this).attr('keyValue'), function( response, status, xhr ) {
	...
	$('a#state-modify').on('click', function(event) {
		$('#dialog-4').load('/cms/module/consultings/apply/stateEdit.do?editMode=STATEMODIFY&homepage_id=' + $('#homepage_id_1').val() + '&apply_idx=' + $(this).attr('keyValue'), function( response, status, xhr ) {
	...
	$('a#delete-btn').on('click', function(event) {
		if(confirm('해당 정보를 삭제 하시겠습니까?')) {
			...
			if(doAjaxPost($('#apply_edit2'))) {
				$('#dialog-3').load('/cms/module/consultings/apply/applyEdit.do?editMode=VIEW&homepage_id=' + $('#homepage_id').val() + '&consultings_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('plan_date'), function( response, status, xhr ) {
...
<form:form modelAttribute="consultingApply" id="apply_edit2" action="/cms/module/consultings/apply/save.do" method="post">
