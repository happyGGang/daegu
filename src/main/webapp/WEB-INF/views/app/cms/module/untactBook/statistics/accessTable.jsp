...
<thead>
	<tr>
		<c:choose>
			<c:when test="${untactBookStatistics.date_type == 'TIME'}">
				<th width="200">시간</th>
			</c:when>
			<c:when test="${untactBookStatistics.date_type == 'DAY'}">
				<th width="200">일</th>
			</c:when>
			<c:when test="${untactBookStatistics.date_type == 'WEEK'}">
				<th width="200">요일</th>
			</c:when>
			<c:when test="${untactBookStatistics.date_type == 'MONTH'}">
				<th width="200">월</th>
			</c:when>
		</c:choose>
		<th>방문수</th>
...
<tbody>
	<c:forEach var="i" varStatus="status" items="${untactBookStatisticsList}">
		<tr>
			<td class="left">
			<c:choose>
				<c:when test="${untactBookStatistics.date_type == 'WEEK'}">
				...
				<c:otherwise>
				${i.result_date}<c:if test="${untactBookStatistics.date_type == 'TIME'}">시</c:if>
				</c:otherwise>
			</c:choose>
			</td>
