...
$(function() {
	...
	$('select.approval_status').on('change', function(e) {
		e.preventDefault();
		$('form#bookReportContest').attr('action', 'statusChange.do');
		$('#book_report_idx').val($(this).data('key'));
		$('#approval_status').val($(this).val());
		doAjaxPost($('form#bookReportContest'));
	});
	
	$('a.delete_btn').on('click', function(e) {
		e.preventDefault();
		if (confirm('정말로 삭제하시겠습니까?\n\n삭제된 신청은 복구가 불가능합니다.')) {
			$('form#bookReportContest').attr('action', 'delete.do');
			$('#book_report_idx').val($(this).data('key'));
			$('#editMode').val('DELETE');
			doAjaxPost($('form#bookReportContest'));
			location.reload();
	...
	$('a#excelDownload').on('click', function(e) {
		$('#bookReportContest').attr('action', 'excelDownload.do').submit();
		$('#bookReportContest').attr('action', 'save.do');
		e.preventDefault();
	});

	$('a#csvDownload').on('click', function(e) {
		e.preventDefault();
		$('#bookReportContest').attr('action', 'csvDownload.do').submit();
	});
	
});
</script>

<form:form modelAttribute="bookReportContest">
			...
			<tbody>
				<c:forEach var="i" varStatus="status" items="${bookReportContestList}">
					<tr>
						...
						<td>
							<a href="#" class="view_btn" data-key="${i.book_report_idx}">${i.user_name}</a>
						</td>
						<td>${i.user_phone}</td>
						<td>${i.school_name}</td>
						<td>${i.user_email}</td>
						...
						<td>
							<select class="approval_status" data-key="${i.book_report_idx}">
						...
						<td>
							<a href="#" class="btn delete_btn" data-key="${i.book_report_idx}">삭제</a>
						</td>
					</tr>
				</c:forEach>
...
<div id="dialog-1" class="dialog-common" title="독후감 공모 참가신청"></div>
