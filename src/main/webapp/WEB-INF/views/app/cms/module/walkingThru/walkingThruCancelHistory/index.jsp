...
$(function(){
	$('button#search_btn').on('click', function(e) {
		e.preventDefault();
		$('#viewPage').val(1);
		doGetLoad('index.do', $('form#walkingThruCancelHistory').serialize());
	});
	...
	}).datepicker('setDate', '${walkingThruCancelHistory.cancel_start_date}');
	...
	}).datepicker('setDate', '${walkingThruCancelHistory.cancel_end_date}');
	...
	$('a#excelDownload').on('click', function(e) {
			$('#walkingThruCancelHistory').attr('action', 'excelDownload.do').submit();
			$('#walkingThruCancelHistory').attr('action', 'save.do')
		e.preventDefault();
	});
	
});
</script>
<form:form id="walkingThruCancelHistory" modelAttribute="walkingThruCancelHistory" method="POST" action="save.do">
<form:hidden id="homepage_id" path="homepage_id"/>

<div class="search">
<label class="blind">검색</label>
	검색 결과 : ${walkingThruCancelHistoryCount}건
	<form:select path="rowCount" class="selectmenu" style="width:150px;">
		...
		<form:option value="${walkingThruCancelHistoryCount}">전체 보기</form:option>
	</form:select>
...
<table class="type1 center">
	...
	<tbody>
	<c:if test="${fn:length(walkingThruCancelHistoryList) < 1}">
		<tr style="height:100%">
			<td colspan="7" style="background:#f8fafb;">워킹스루 도서대출 취소내역이 없습니다.</td>
		</tr>
	</c:if>
	<c:forEach var="i" varStatus="status" items="${walkingThruCancelHistoryList}">
		<tr>
...
<jsp:include page="/WEB-INF/views/app/cms/common/paging.jsp" flush="false">
	<jsp:param name="formId" value="#walkingThruCancelHistory"/>
</jsp:include>
