...
$(function() {
		...
		buttons: [
			{
				...
				click: function() {
					...
					var homepage_id 	= '${eventQuestion.homepage_id}';
					var event_idx 		= '${eventQuestion.event_idx}';
					var question_form 	= [];
					question_form.push('<div class="questionLayer" style="margin-bottom:10px;">');
					question_form.push('<form id="' + control_key + '" name="eventQuestion" class="questionForm" method="post" action="saveQuestion.do" >');
					...
					question_form.push('	<input type="hidden" id="event_idx" name="event_idx" value="' + event_idx + '"/>');
					question_form.push('	<input type="hidden" id="event_question_idx" name="event_question_idx" value="0"/>');
					question_form.push('    <input type="hidden" id="event_question_item" name="event_question_item"/>');
					...
					question_form.push('	         		<select name="event_question_type" class="selectmenu" keyValue="' + control_key + '">');
					...
					question_form.push('	         	<td><textarea name="event_question_title" class="text" style="width:100%" value="${i.event_question_title}"></textarea></td>');
					question_form.push('	        </tr>');
					question_form.push('	        <tr>');
					...
					question_form.push('	        	<td><input name="event_question_answer" class="text" />');
				...
				click: function() {
					...
					if ( doValidation() ) {
						$('form.questionForm').each(function(i, v) {
							...
							form.find('input[name="event_question_item"]').val(items.join('|'));
	...
	function doValidation() {
		var isValid = true;
		$('select[name="event_question_type"]').each(function() {
			if ( $(this).val() == '' ) {
				isValid = false;
				$(this).focus();
				alert('문항 구분을 지정하세요.');
		...
		$('input[name="event_question_title"]').each(function() {
			if ( $(this).val() == '' ) {
				isValid = false;
				$(this).focus();
				alert('문항 제목을 입력하세요.');
	...
	function reload() {
		$('#dialog-2').load('editQuestion.do?homepage_id=${eventQuestion.homepage_id}&event_idx=${eventQuestion.event_idx}');
	}
...
<div id="questionArea">
	<c:choose>
		<c:when test="${fn:length(eventQuestionList) > 0}">
			<c:forEach items="${eventQuestionList}" var="i" varStatus="status">
				<div class="questionLayer" style="margin-bottom:10px;">
					<form:form id="question_${status.index}" class="questionForm" modelAttribute="eventQuestion" method="post" action="saveQuestion.do" >
						<form:hidden path="editMode" value="MODIFY"/>
						<form:hidden path="homepage_id" value="${i.homepage_id}"/>
						<form:hidden path="event_idx" value="${i.event_idx}"/>
						<form:hidden path="event_question_idx" value="${i.event_question_idx}"/>
						<form:hidden path="event_question_item"/>
						<table class="type2">
							<colgroup>
						       <col width="130" />
						       <col width="*"/>
					       	...
					       	<tbody>
					       		<tr>
						         	<th>문항 구분</th>
						         	<td>
						         		<select name="event_question_type" class="selectmenu" keyValue="question_${status.index}">
						         			<option value="RADIO" <c:if test="${i.event_question_type eq 'RADIO'}">selected="selected"</c:if>>한개선택</option>
						         			<option value="CHECK" <c:if test="${i.event_question_type eq 'CHECK'}">selected="selected"</c:if>>다중선택</option>
						         			<option value="TEXT" <c:if test="${i.event_question_type eq 'TEXT'}">selected="selected"</c:if>>주관식</option>
						         		</select>
						         	</td>
						        </tr>
						        <tr>
						         	<th>문항 제목</th>
						         	<td><textarea name="event_question_title" style="width:100%;" >${i.event_question_title}</textarea></td>
						        </tr>
						        <tr>
						         	<th>문항 보기 <a class="btn btn1 add-item" keyValue="question_${status.index}">추가</a></th>
						         	<td class="question_items">
						         		<c:forEach items="${fn:split(i.event_question_item,'|')}" var="j" >
						        ...
						        <tr>
						         	<th>정답</th>
						         	<td><input name="event_question_answer" class="text" value="${i.event_question_answer}"/>
						         		<div class="ui-state-highlight">
		...
		<c:otherwise>
			<h3 style="color:#666;" id="comment"><b>등록된 이벤트가 없습니다.</b></h3>
		</c:otherwise>
	</c:choose>
