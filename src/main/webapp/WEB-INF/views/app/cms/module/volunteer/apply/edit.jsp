...
$(function() {
	...
	if('${volunteerApply.applicant_tel}' == '') {
		$('#applicant_tel_1').val("010");
	} else {
		var applicant_tel = '${volunteerApply.applicant_tel}'.split('-');
		$('#applicant_tel_1').val(applicant_tel[0]);
		$('#applicant_tel_2').val(applicant_tel[1]);
		$('#applicant_tel_3').val(applicant_tel[2]);
	}

	if('${volunteerApply.guide_tel}' == '') {
		$('#guide_tel_1').val("010");
	} else {
		var guide_tel = '${volunteerApply.guide_tel}'.split('-');
		$('#guide_tel_1').val(guide_tel[0]);
		$('#guide_tel_2').val(guide_tel[1]);
		$('#guide_tel_3').val(guide_tel[2]);
	}

	if('${volunteerApply.protector_tel}' == '') {
		$('#protector_tel_1').val("010");
	} else {
		var protector_tel = '${volunteerApply.protector_tel}'.split('-');
	...
	$('a.idCheck').on('click', function(e) {
		...
		$.get('/cms/module/volunteer/apply/checkId.do?homepage_id=' + $('#homepage_id').val() + '&applicant_member_id='+ $('#applicant_member_id').val() + '&search_api_type=' + $('#search_api_type').val(), function(response) {
...
<form:form modelAttribute="volunteerApply" id="apply_edit" action="/cms/module/volunteer/apply/save.do" method="post" onsubmit="return false;">
<form:hidden path="editMode"/>
<form:hidden path="homepage_id"/>
<form:hidden path="apply_idx"/>
<form:hidden path="use_time"/>
<form:hidden path="start_time" value="${volunteerApply.start_time }"/>
<form:hidden path="end_time" value="${volunteerApply.end_time }"/>
<form:hidden path="volunteer_idx" value="${volunteerApply.volunteer_idx }"/>
<form:hidden path="start_date" value="${volunteerApply.start_date }"/>
<table class="type2">
	<colgroup>
		<col width="140"/>
		<col width="*"/>
	...
	<tbody>
		<tr>
         	...
         	<td>
         		<c:choose>
         			<c:when test="${volunteerApply.editMode eq 'ADD' }">
         			...
         			<c:otherwise>
         				${volunteerApply.applicant_member_id}
		...
		<c:if test="${volunteerApply.homepage_id eq 'h50'}">
		<tr>
		...
		<c:choose>
		<c:when test="${volunteerApply.homepage_id ne 'h50'}">
			<tr>
