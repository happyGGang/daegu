...
$(function(){
	...
	$('a.modify').on('click', function(event) {
		...
		$('#dialog-1').load('edit.do?editMode=MODIFY&homepage_id=' + $('#homepage_id_1').val() + '&showPerformance_idx=' + $(this).attr('keyValue'), function( response, status, xhr ) {
			$('#dialog-1').dialog('open');
		});
	...
	$('a#apply').on('click', function(event) {
		$('#dialog-2').load('/cms/module/showPerformance/apply/edit.do?editMode=ADD&homepage_id=' + $('#homepage_id_1').val() + '&showPerformance_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2') +'&start_time=' + $(this).attr('keyValue3') +'&end_time=' + $(this).attr('keyValue4'), function( response, status, xhr ) {
	...
	$('a#noMemberApply').on('click', function(event) {
		$('#dialog-2').load('/cms/module/showPerformance/apply/noMemberApply.do?editMode=ADD&homepage_id=' + $('#homepage_id_1').val() + '&showPerformance_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2') +'&start_time=' + $(this).attr('keyValue3') +'&end_time=' + $(this).attr('keyValue4'), function( response, status, xhr ) {
			$('#dialog-2').dialog('open');
		});
	...
	$('a#check_apply').on('click', function(event) {
		$('#dialog-3').load('/cms/module/showPerformance/apply/applyEdit.do?editMode=ADD&homepage_id=' + $('#homepage_id_1').val() + '&showPerformance_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2') +'&start_time=' + $(this).attr('keyValue3') +'&end_time=' + $(this).attr('keyValue4'), function( response, status, xhr ) {
	...
	$('a#excelDownload').on('click', function(event) {
		...
		$('#showPerformance').attr('action','/cms/module/showPerformance/apply/excelDownloadMonth.do').submit();
	});
	...
	$('a#totalExcelDownload').on('click', function(event) {
		...
		$('#plan_date').val("");
		$('#showPerformance').attr('action','/cms/module/showPerformance/apply/excelDownload.do').submit();
	});
	...
	$('a#csvDownload').on('click', function(e) {
		...
		$('#showPerformance').attr('action', '/cms/module/showPerformance/apply/csvDownloadMonth.do').submit();
	});
	...
	$('a#totalCsvDownload').on('click', function(e) {
		...
		$('#plan_date').val("");
		$('#showPerformance').attr('action', '/cms/module/showPerformance/apply/csvDownload.do').submit();
	});
	...
	$('a#allChecked').on('click', function(e) {
		...
		$('input[name = showPerformance_idx_arr]').prop('checked', allChecked);
	});
	
	$('a#checkedDelete').on('click', function(e) {	
		e.preventDefault();
		if ($('input[name=showPerformance_idx_arr]:checked').length < 1) {
			alert('선택된 일자가 없습니다.');
			return false;
		}
		if(confirm('선택된 신청을 삭제 하시겠습니까?\n삭제시 신청자 정보등 모든 관련된 정보도 같이 삭제되며 복구 불가능합니다.')){
			$('form#showPerformance').attr('action','save.do');
			$('form#showPerformance').attr('method','post');
			$('form#showPerformance input#editMode').val('BATCHDELETE');
			if (doAjaxPost($('form#showPerformance'))) {
				location.reload();
			}
		}
	});
...
<form:form modelAttribute="showPerformance" action="" id="showPerformance">
...
<div class="infodesk">
	...
	<div class="button">
		...
		<c:if test="${authC}">
		<a href="" class="btn btn5 left" id="dialog-add"><i class="fa fa-plus"></i><span>공연가능일 관리</span></a>
		</c:if>
	</div>
</div>
				...
				<tr>
					...
					<c:otherwise>
						<td class="top" style="vertical-align: top;">
							...
							<tag:showPerformance plan_date="${plan_date}" showPerformanceList="${showPerformanceList}" mode="admin" />
						</td>
					</c:otherwise>
					</c:choose>
					<c:choose>
