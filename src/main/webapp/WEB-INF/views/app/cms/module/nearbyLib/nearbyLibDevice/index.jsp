...
$(function() {
	...
	$('a#dialog-modify').on('click', function(e) {
		$('#dialog-1').load('edit.do?editMode=MODIFY&device_idx=' + $(this).attr('keyValue1') + '&device_code=' + $(this).attr('keyValue2'), function( response, status, xhr ) {
			$('#dialog-1').dialog('open');
		});
		e.preventDefault();
	});
	...
	$('a#delete-btn').on('click', function(e) {
		if ( confirm($(this).attr('keyValue2') + ' 를(을) 삭제 하시겠습니까?')) {
			...
			$('#deleteDevice #device_code').val($(this).attr('keyValue3'));
	...
	$('a#locker-edit').on('click', function(e) {
		...
		$('#dialog-2').load('/cms/module/nearbyLibLocker/edit.do?editMode=ADD&device_idx=' + $(this).attr('keyValue1') + '&device_code=' + $(this).attr('keyValue2') + '&device_name=' + $(this).attr('keyValue3'), function( response, status, xhr ) {
	...
	$('a#locker-detail').on('click', function(e) {
		$('#dialog-3').load('/cms/module/nearbyLibLocker/detail.do?device_idx=' + $(this).attr('keyValue'), function( response, status, xhr ) {
	...
	$('a#locker-index').on('click', function(e) {
			var win = window.open('/cms/module/nearbyLibLocker/index.do?editMode=ADD&device_idx=' + $(this).attr('keyValue1') + '&device_name=' + $(this).attr('keyValue2'), '_blank');
	        win.focus();					
		e.preventDefault();
	});
...
<form:form id="deleteDevice" modelAttribute="nearbyLibDevice" action="save.do">
<form:hidden path="editMode" value="DELETE"/>
<form:hidden path="device_idx"/>
<form:hidden path="device_code"/>
</form:form>
<form:form  modelAttribute="nearbyLibDevice" id="Device">
...
<%-- 					<td>
					...
					<td>						
						<a href="#" class="btn" id="dialog-modify" keyValue1="${i.device_idx }" keValue2="${i.device_code }" >수정</a>											
						<a href="#" class="btn" id="delete-btn" keyValue1="${i.device_idx }" keyValue2="${i.device_name }" keyValue3="${i.device_code }">삭제</a>						
					</td>
				</tr>
			</c:forEach>
...
<div id="dialog-1" class="dialog-common" title="운영장비 관리"></div>
<div id="dialog-2" class="dialog-common" title="사물함 등록"></div>
<div id="dialog-3" class="dialog-common" title="사물함 정보"></div>
