...
$(function() {
	...
	$('a.dialog-edit').on('click', function(e) {
		e.preventDefault();
		$('#dialog-2').load('loanEdit.do?editMode=MODIFY&library_check_loan_idx='+$(this).attr('keyValue'), function(response, status, xhr) {
	...
	$('select#library_check_number, select#request_status, select#rowCount').on('change', function() {
		$('#viewPage').val(1);
		doGetLoad('loanList.do', $('form#libraryCheck').serialize());
	});
...
<%-- 							<c:when test="${i.request_status eq '5'}">반납요청완료</c:when> --%>
					...
					<td>
						<c:if test="${i.request_status eq '0' or i.request_status eq '1' or i.request_status eq '6'}">
