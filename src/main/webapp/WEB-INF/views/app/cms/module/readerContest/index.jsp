...
$(function() {
	...
	$('select#rowCount').on('change', function() {
		$('#viewPage').val(1);
		doGetLoad('index.do', $('form#readerContest').serialize());
	});
	...
	$('a.view_btn').on('click', function(e){
		e.preventDefault();
		$('#reader_idx').val($(this).data('key'));
		doGetLoad('view.do', serializeCustom($('form#readerContest')));
	});
	
	$('select.approval_status').on('change', function(e) {
		e.preventDefault();
		$('form#readerContest').attr('action', 'statusChange.do');
		$('#reader_idx').val($(this).data('key'));
		$('#approval_status').val($(this).val());
		doAjaxPost($('form#readerContest'));
	});
	
	$('a.delete_btn').on('click', function(e) {
		e.preventDefault();
		if (confirm('정말로 삭제하시겠습니까?\n\n삭제된 신청은 복구가 불가능합니다.')) {
			$('form#readerContest').attr('action', 'delete.do');
			$('#reader_idx').val($(this).data('key'));
			$('#editMode').val('DELETE');
			doAjaxPost($('form#readerContest'));
			location.reload();
...
<form:hidden path="approval_status"/>
	...
	<div>
		<table class="type1 center">
			...
			<tbody>
				<c:forEach var="i" varStatus="status" items="${readerContestList}">
					<tr>
						...
						<td>
							<select class="approval_status" data-key="${i.reader_idx}">
						...
						<td>
							<a href="#" class="btn delete_btn" data-key="${i.reader_idx}">삭제</a>
						</td>
					</tr>
				</c:forEach>
				<c:if test="${fn:length(readerContestList) < 1}">
					<tr>
		...
		<jsp:include page="/WEB-INF/views/app/cms/common/paging.jsp" flush="false">
			<jsp:param name="formId" value="#readerContest"/>
		</jsp:include>
...
<div id="dialog-1" class="dialog-common" title="다독자공모 참가신청"></div>
