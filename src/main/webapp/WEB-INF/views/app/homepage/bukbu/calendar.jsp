...
$(function() {
	...
	$('a#before-btn').on('click', function(e) {
		...
		$('div#planList').load('calendar2.do', 'plan_date='+plan_date.format('yyyy-MM'));
		e.preventDefault();
	});
	...
	<h3>${fn:split(calendar.plan_date, '-')[1]}월 휴관일</h3>

	<c:if test="${closeDayList.dd eq ''}">
		등록된 휴일이 없습니다.
	</c:if>
	<c:if test="${closeDayList.dd ne ''}">
	<c:forTokens var="item" items="${closeDayList.dd}" delims=",">
		<span>${item}</span>
	</c:forTokens>
