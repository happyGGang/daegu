...
$(function() {
	...
	$('#save-btn').on('click', function() {
		...
		$.ajax({
			...
			async : false,
			data : serializeObject($('#mediaFactoryEdit')),
			method : 'POST',
			dataType : 'json',
			success : function(data) {
				if(data.valid) {
					...
					if($('#pageType').val() == 'ajax') {
						$('#tabCon2').load('module/mediaFactory/index.do?pageType=ajax');
					} else {
						doGetLoad('/${homepage.context_path}/module/mediaFactory/index.do', '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val());
					}
				} else {
	   				if(data.targetOpener) {
						window.open(data.url, '', 'width=500,height=510');
	...
	$('#cancel-btn').on('click', function() {
		var url = '/${homepage.context_path}/module/mediaFactory/index.do';
...
<form:hidden path="mediaFactory_idx" value="${apply.mediaFactory_idx }"/>
