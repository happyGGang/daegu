...
$(function(){
	...
	$('a#monthSelect').on('click', function(e) {
		...
		if($('#pageType').val() == 'ajax') {
			$('#volunteer #pageType').val('ajax');
			$('#tabCon2').load('module/volunteer/index.do?'+serializeCustom($('#volunteer')));
		} else {
	...
	$('a#apply').on('click', function(event) {
		if($('#pageType').val() == 'ajax') {
			$('#tabCon2').load('/${homepage.context_path}/module/volunteer/edit.do', 'editMode=ADD&volunteer_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2') + '&start_time=' + $(this).attr('keyValue3') +'&end_time=' + $(this).attr('keyValue4') + '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val()+'&homepage_id=' + $('input#homepage_id_1').val());
		} else {
			doGetLoad('/${homepage.context_path}/module/volunteer/edit.do', 'editMode=ADD&volunteer_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2') + '&start_time=' + $(this).attr('keyValue3') +'&end_time=' + $(this).attr('keyValue4') + '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val() + '&date_type=' + $('#date_type').val()+'&homepage_id=' + $('input#homepage_id_1').val());
		}
	...
	function setMonthSelect() {
		var plan_date = '${volunteer.plan_date}';
		for(var i=1; i<= 12; i++) {
	...
	var a = '${fn:escapeXml(volunteer.homepage_id)}';
...
<form:form modelAttribute="volunteer">
				...
				<tr class="week">
					...
					<c:otherwise>
						<td class="sun">
							...
							<ul>
								<tag:volunteerUser member_id="${member_id}" plan_date="${plan_date}" volunteerList="${volunteerList}" calendarManageList="${calendarManageList}" applyList="${applyList}" mode="admin" />
							</ul>
						</td>
					</c:otherwise>
					</c:choose>
