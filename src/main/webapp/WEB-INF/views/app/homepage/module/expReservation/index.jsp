...
$(function(){
	...
	$('a#monthSelect').on('click', function(e) {
		...
		if($('#pageType').val() == 'ajax') {
			$('#expReservation #pageType').val('ajax');
			$('#tabCon2').load('module/expReservation/index.do?'+serializeCustom($('#expReservation')));
		} else {
 ...
 */
	...
	$('a#apply').on('click', function(e) {
		if($('#pageType').val() == 'ajax') {
			$('#tabCon2').load('/${homepage.context_path}/module/expReservation/edit.do', 'editMode=ADD&program_list_idx=' + $(this).attr('keyValue') + '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val() + '&homepage_id=' + $('#homepage_id').val());
		} else {
			doGetLoad('/${homepage.context_path}/module/expReservation/edit.do', 'editMode=ADD&program_list_idx=' + $(this).attr('keyValue') + '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val() + '&date_type=' + $('#date_type').val() + '&homepage_id=' + $('#homepage_id').val());
		}
	...
	function setMonthSelect() {
		var plan_date = '${expReservation.plan_date}';
		for(var i=1; i<= 12; i++) {
...
<form:form modelAttribute="expReservation">
				...
				<tr class="week">
					...
					<c:otherwise>
						<td class="sun">
							...
							<ul>
								<tag:expReservationUser plan_date="${plan_date}" expReservationList="${expReservationList}" calendarManageList="${calendarManageList}" expApplyList="${expApplyList}" mode="admin"/>
							</ul>
						</td>
					</c:otherwise>
					</c:choose>
