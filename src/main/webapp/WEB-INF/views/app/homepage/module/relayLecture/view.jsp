...
<form:hidden path="lecture_idx"/>
	...
	<table class="type1">
		...
		<tbody>
			...
			<tr>
				<td class="center" colspan="4" style="height: 100px; padding: 20px 0; vertical-align: top;">
					<div style="margin-bottom: 20px;">
						<c:choose>
								<c:when test="${getRelayLecture.apply_status eq '0'}">
									<span class="btn btn4" style="padding: 10px; font-size: 20px;">신청 대기 기간입니다.</span>
								</c:when>
								<c:when test="${getRelayLecture.apply_status eq '1'}">
									<a href="#" class="btn btn1 apply_btn" data-key="${i.lecture_idx}" style="padding: 10px; font-size: 20px;">강연 신청하기</a>
								</c:when>
								<c:when test="${getRelayLecture.apply_status eq '2'}">
									<span class="btn btn5" style="padding: 10px; font-size: 20px;">접수가 마감 되었습니다.</span>
								</c:when>
								<c:when test="${getRelayLecture.apply_status eq '3'}">
									<span class="btn btn6" style="padding: 10px; font-size: 20px;">신청가 마감 되었습니다.</span>
								</c:when>
							</c:choose>
					</div>
