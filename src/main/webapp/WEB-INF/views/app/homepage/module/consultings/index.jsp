...
$(function(){
	...
	$('a#monthSelect').on('click', function(e) {
		...
		if($('#pageType').val() == 'ajax') {
			$('#consultings #pageType').val('ajax');
			$('#tabCon2').load('module/consultings/index.do?'+serializeCustom($('#consultings')));
		} else {
	...
	$('a#apply').on('click', function(event) {
		if($('#pageType').val() == 'ajax') {
			$('#tabCon2').load('/${homepage.context_path}/module/consultings/edit.do', 'editMode=ADD&consultings_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2') + '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val()+'&homepage_id=' + $('input#homepage_id_1').val());
		} else {
			doGetLoad('/${homepage.context_path}/module/consultings/edit.do', 'editMode=ADD&consultings_idx=' + $(this).attr('keyValue') + '&start_date=' + $(this).attr('keyValue2') + '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val() + '&date_type=' + $('#date_type').val()+'&homepage_id=' + $('input#homepage_id_1').val());
		}
	...
	function setMonthSelect() {
		var plan_date = '${consultings.plan_date}';
		for(var i=1; i<= 12; i++) {
	...
	var a = '${fn:escapeXml(consultings.homepage_id)}';
...
<form:form modelAttribute="consultings">
				...
				<tr class="week">
					...
					<c:otherwise>
						<td class="sun">
							...
							<ul>
								<tag:consultingsUser plan_date="${plan_date}" consultingsList="${consultingsList}" calendarManageList="${calendarManageList}" applyList="${applyList}" mode="admin"/>
							</ul>
						</td>
					</c:otherwise>
					</c:choose>
