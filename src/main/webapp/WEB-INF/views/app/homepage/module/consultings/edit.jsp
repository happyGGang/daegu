...
$(function() {
	...
	$('#save-btn').on('click', function() {
		...
		$.ajax({
			url : '/${homepage.context_path}/module/consultings/save.do',
			async : false,
			data : serializeObject($('#consultingsEdit')),
			method : 'POST',
			dataType : 'json',
			success : function(data) {
				if(data.valid) {
					...
					if($('#pageType').val() == 'ajax') {
						$('#tabCon2').load('module/consultings/index.do?pageType=ajax');
					} else {
						doGetLoad('/${homepage.context_path}/module/consultings/index.do', '&menu_idx=' + $('#menu_idx').val() + '&pageType=' + $('#pageType').val());
					}
				} else {
	   				if(data.targetOpener) {
						window.open(data.url, '', 'width=500,height=510');
	...
	$('#cancel-btn').on('click', function() {
		var url = '/${homepage.context_path}/module/consultings/index.do';
...
<form:form modelAttribute="consultingApply" id="consultingsEdit" action="/${homepage.context_path}/module/consultings/save.do" method="post" onsubmit="return false;">
...
<form:hidden path="consultings_idx" value="${apply.consultings_idx }"/>
...
<table class="type1">
	...
	<tbody>
		...
		<tr>
			<th>신청자 주소</th>
			<td>
				<form:input path="applicant_address" class="text" cssStyle="width:250px"/><button class="btn btn2 findPostCode" keyValue1="#applicant_zipcode" keyValue2="#applicant_address" keyValue3="#age">주소 찾기</button>
			</td>
		</tr>
		<tr>
			<th>신청자 연령대(<span style="color: red; font-weight: bold;">*</span>)</th>
			<td>
				<form:input path="age" class="text" cssStyle="width:50px" />
			</td>
		</tr>
		<tr>
			<th>상담인원(<span style="color: red; font-weight: bold;">*</span>)</th>
