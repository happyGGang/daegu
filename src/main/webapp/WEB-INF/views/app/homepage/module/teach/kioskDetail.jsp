	...
	<div class="contents">
		<div class="">
			...
			<div class="culturedetail-request-button-box">
				<a href="" class="button2">수강신청하기</a>
			</div>
			
			<c:choose>
				<c:when test="${teach.teach_status eq '0'}">
					<a href="" class="btn btn1 add" keyValue1="${teach.homepage_id}" keyValue2="${teach.group_idx}" keyValue3="${teach.category_idx}" keyValue4="${teach.teach_idx}" keyValue5="${teach.large_category_idx}" apply_status="1">수강신청</a>
				</c:when>
				<c:when test="${teach.teach_status eq '1'}">
					<a href="" class="btn btn1 add" keyValue1="${teach.homepage_id}" keyValue2="${teach.group_idx}" keyValue3="${teach.category_idx}" keyValue4="${teach.teach_idx}" keyValue5="${teach.large_category_idx}" apply_status="2">대기자신청</a>
				</c:when>
				<c:when test="${teach.teach_status eq '2' or i.teach_status eq '10'}">
					<a href="/${homepage.context_path}/module/teach/applyList.do?menu_idx=${myTeachListMenuIdx}" class="btn btn2">신청완료</a>
				</c:when>
				<c:when test="${teach.teach_status eq '3'}">
					<a href="/${homepage.context_path}/module/teach/applyList.do?menu_idx=${myTeachListMenuIdx}" class="btn btn2">대기자 신청완료</a>
				</c:when>
				<c:when test="${teach.teach_status eq '9'}">
					<a href="javascript:void(0);" class="btn" style="cursor: default;">수강종료</a>
				</c:when>
				<c:when test="${teach.teach_status eq '4'}">
					<a href="javascript:void(0);" class="btn" style="cursor: default;">접수마감</a>
				</c:when>
				<c:when test="${teach.teach_status eq '5'}">
					<a href="javascript:void(0);" class="btn" style="cursor: default;">정원마감</a>
				</c:when>
				<c:when test="${teach.teach_status eq '6'}">
					<a href="javascript:void(0);" class="btn btn4" style="cursor: default;">신청대기</a>
				</c:when>
			</c:choose>
		</div>
	</div>
