...
$(function(){
	...
	$('a.cancel').on('click', function(e) {
		...
		if (confirm("해당 강좌 신청을 취소하시겠습니까? 취소후 해당 강좌에 대해 재신청 가능합니다.")) {
			...
			$('input#editMode').val('ANONYCANCEL');
	...
	$('a#search-btn').on('click', function(e) {
		e.preventDefault();
		doGetLoad('anonyApplyList.do', $('form#teach').serialize());
	});
});
</script>
...
<%-- 									<label>수강생</label> : ${i.student_name} ( ${i.student_sex eq 'M' ? '남' : '여'} ) --%>
									<c:if test="${i.status eq '1' and i.wait_num != 0}">
										<span>${i.wait_num}번째 참여자로 신청되었습니다.</span>
									</c:if>
									<c:if test="${i.status eq '2'and i.wait_num != 0}">
