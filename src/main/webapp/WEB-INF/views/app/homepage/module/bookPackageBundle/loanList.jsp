...
$(function() {
	...
	$('a.btn-view').on('click', function(e) {
		e.preventDefault();
		var formData = 'menu_idx='+$('#menu_idx').val() + '&book_package_bundle_loan_idx='+$(this).attr('keyValue') + '&viewPage='+$('#viewPage').val();
	...
	$('a.return-req').on('click', function(e) {
		...
		$('form#bookPackageBundle').attr('action', 'loanSave.do');
		$('#book_package_bundle_loan_idx').val($(this).attr('keyValue'));
	...
	var currYear = '${bookPackageBundle.loan_start_date}';
	...
	$('input[name="return_yn"]').on('click', function() {
		$('#viewPage').val(1);
		doGetLoad('loanList.do', $('form#bookPackageBundle').serialize());
	});
	...
	$('#status-change').on('click', function(e) {
		...
		if($('input[name="book_package_bundle_loan_arr"]:checked').length < 1) {
		...
		$('#editMode').val('STATUS');
		$('#bookPackageBundle').attr('action', 'loanSave.do');
	...
	$('a#excelDownload').on('click', function(e) {
		...
		if('${fn:length(loanList)}' > 0) {
			...
			$('#bookPackageBundle').attr('action', 'loanList.do');
		} else {
			alert('해당 내역이 없습니다.');
		}
	});
...
$(function() {
    $('.tabmenu a').on('click',function() { 
          ...
          if ( key == 'tabCon1' ) {
        	  doGetLoad('../bookPackage/loanList.do?menu_idx=${fn:escapeXml(param.menu_idx)}');
          }
          else if ( key == 'tabCon2') {
          }
    });
...
<div class="tabmenu on tab1">
    <ul>
        <li><a title="대출현황조회(책·미니·주제)" href="#tabCon1" keyvalue="tabCon1">대출현황조회(책·미니·주제)</a></li>
        <li class="active"><a title="대출현황조회(학생추천도서)" href="#tabCon2" keyvalue="tabCon2">대출현황조회(학생추천도서)</a></li>
    </ul>
</div>
...
<form:hidden path="book_package_bundle_loan_idx"/>
	...
	<div class="search txt-center">
		<fieldset>
			<form:select path="search_type" cssClass="selectmenu">
				<form:option value="book_package_bundle_title">책꾸러미명</form:option>
		...
		<tbody>
			<c:forEach var="i" varStatus="status" items="${loanList}">
				<tr>
					<c:if test="${member.admin or loginSupport.auth_group eq '1'}">
					<td>
						<input type="checkbox" name="book_package_bundle_loan_arr" class="loan_chk" value="${i.book_package_bundle_loan_idx}"/>
					</td>
					</c:if>
					<td class="num">${paging.listRowNum - status.index}</td>
					<td class="left">
						<a href="#" class="btn-view" keyValue="${i.book_package_bundle_loan_idx}">${i.book_package_bundle_title}</a>
						<br/>
					...
					<td>
						...
						<c:choose>
							...
							<a href="#" class="return-req sub-btn return" keyValue="${i.book_package_bundle_loan_idx}" keyValue2="Y">반납요청</a>
							</c:when>
							<c:otherwise>
							<a href="#" class="return-req sub-btn return2" keyValue="${i.book_package_bundle_loan_idx}" keyValue2="N">반납요청중</a>
							</c:otherwise>
						</c:choose>
						</c:if>
					</td>
					<td>
						...
						<a href="#" class="cancle-btn sub-btn" keyValue="${i.book_package_bundle_loan_idx}">취소</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
