...
$(function() {
	...
	$('div.tab_menu a').on('click', function(e) {
		...
		doGetLoad('apply.do', serializeCustom($('#showApplyEdit')));
	});
	</c:if>
...
<form:form modelAttribute="showApply" id="showApplyEdit" action="/${homepage.context_path}/module/showPerformance/save.do" method="post">
				...
				<tr>
					...
					<td>
						...
						<c:if test="${apply_state ne '3'}">
							<a href="" class="btn" id="delete-btn" keyValue="${i.showPerformance_idx}">신청취소</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
			<c:if test="${fn:length(showApplyList) < 1}">
				<tr>
