...
$(function() {
	...
	$('#save-btn').on('click', function(e) {
		...
		doAjaxPost($('#bookPackageBundleLoan'));
	});
...
<form:form id="bookPackageBundleLoan" modelAttribute="bookPackageBundle" action="loanSave.do" method="POST">
			...
			<tr>
				...
				<c:choose>
					<c:when test="${loginSupport.auth_group eq '3'}">
						...
						<c:if test="${bookPackageBundle.request_status > 0}">
							<form:hidden path="request_status" value="1"/>예약상담중
						</c:if>
					</c:when>
					<c:otherwise>
...
<br/>
<c:if test="${bookPackageBundle.editMode eq 'ADD'}">
<div>
