...
<script>
	...
	function yesb_read(url, lend_idx,book_idx,user_idx,book_code) {
		...
		add_viewer_access(lend_idx,book_idx,user_idx,book_code, 'YESB');
	}

	function fxli_read(lend_idx,book_idx,user_idx,book_code) {
		$('input#book_num').val(book_code);
		...
		add_viewer_access(lend_idx,book_idx,user_idx,book_code, 'FXLI');
		$('form#frm_fx').submit();
	}
	...
	function add_viewer_access(lend_idx,book_idx,user_idx,book_code, viewer_type) {
		var ajaxData = {
			'lend_idx' : lend_idx,
			'book_idx' : book_idx,
			'user_idx' : user_idx,
		...
		$.ajax({
			...
			success: function(response) {
				...
				console.log('[' + textStatus + ']관리자에게 문의하세요. : ' + errorThrown);
			}
		});
	}

	function checkApp(url, com_code, lend_idx,book_idx,user_idx,book_code) {
        ...
        */
		...
		if (isIphone) {
			if(confirm('뷰어앱이 설치되어 있으면 확인(승인)을 클릭하시고,\n설치되어 있지 않다면 취소를 클릭하세요. (앱스토어 이동)')) {
				add_viewer_access(lend_idx,book_idx,user_idx,book_code,com_code);
								...
								<c:choose>
									<c:when test="${i.com_code == 'FXLI'}">
										...
										<c:choose>
											...
											<c:otherwise>
												<c:set var="read" value="fxli_read('${i.lend_idx}','${i.book_idx}','${i.user_idx}','${i.book_code}'); return false;"/>
											</c:otherwise>
										</c:choose>
									</c:when>
