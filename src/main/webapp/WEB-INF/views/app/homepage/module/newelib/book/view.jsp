	...
	$('a#book_borrow').on('click', function(e) {
		...
		if(doAjaxPost($form)) {
			if(confirm('지금 대출 목록을 확인하시겠습니까?')) {
				location.href = '/${homepage.context_path}/module/newelib/lending/index.do?menu_idx=39&menu=LENDING'
			}
		}
		$form.prop('action', 'index.do');
	});
	...
	$('a#book_reserve').on('click', function(e) {
		...
		if(doAjaxPost($form)) {
			if(confirm('지금 예약 목록을 확인하시겠습니까?')) {
				location.href = '/${homepage.context_path}/module/newelib/lending/index.do?menu_idx=48&menu=RESERVE'
			}
		}
		$form.prop('action', 'index.do');
	});
	...
	$('a#book_addfavorite').on('click', function(e) {
		...
		if(doAjaxPost($form)) {
			if(confirm('지금 나의 관심도서 목록을 확인하시겠습니까?')) {
				location.href = '/${homepage.context_path}/module/newelib/lending/index.do?menu_idx=78&menu=MYSTUDY'
			}
		}
		$form.prop('action', 'index.do');
	});
