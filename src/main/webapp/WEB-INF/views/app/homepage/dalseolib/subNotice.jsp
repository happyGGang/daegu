...
<c:forEach items="${subNoticeList}" var="i" varStatus="status" begin="0" end="5">
    ...
    <c:if test="${i.category5 eq 'h69'}"><c:set var="libcode" value="lib04"></c:set></c:if>
    <c:if test="${i.category5 eq 'h66'}"><c:set var="libcode" value="lib05"></c:set></c:if>
    <c:if test="${i.category5 eq 'h70'}"><c:set var="libcode" value="lib06"></c:set></c:if>
    <li class="${libcode}">
