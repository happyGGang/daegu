...
<div id="calendar2">
	...
	<div class='calendar-wrap'>
		<div class='cal-bord'>
			<div class="cal-func2">
				<div class="cal-btn">
					<a id="before-btn" href="#prev" class="btn prev" keyValue="${calendar.plan_date}"><img src="/resources/homepage/${homepage.context_path}/img/cal-prev-btn.png" alt="이전달"><span class="blind">이전달</span></a>
					<b class="date"><span class="months">${fn:split(calendar.plan_date, '-')[1]}</span><span class="years">${fn:split(calendar.plan_date, '-')[0]}</span></b>
					<a id="next-btn" href="#next" class="btn next" keyValue="${calendar.plan_date}"><img src="/resources/homepage/${homepage.context_path}/img/cal-next-btn.png" alt="다음달"><span class="blind">다음달</span></a>
				</div>

				<div class="cal-btn-more">
					<a href="/${homepage.context_path}/module/calendarManage/index.do?menu_idx=36">일정 한 눈에 보기</a>
				</div>
			</div>
						...
						<div class="planList">
							<c:choose>
								<c:when test="${fn:indexOf(closeDayList.dd, key) > -1 }">
									<p class="datetime holiday"><!-- 루프돌때, 해당 행사가 휴관이면 holiday 클래스를 추가, 행사면 event 클래스를 추가 -->
										<span class="years">${fn:substring(calendar.plan_date,0,4)}</span>
										<span class="monthDays">${fn:substring(calendar.plan_date,5,7)}.${key}</span>
									</p>
								</c:when>
								<c:otherwise>
									<p class="datetime event"><!-- 루프돌때, 해당 행사가 휴관이면 holiday 클래스를 추가, 행사면 event 클래스를 추가 -->
										<span class="years">${fn:substring(calendar.plan_date,0,4)}</span>
										<span class="monthDays">${fn:substring(calendar.plan_date,5,7)}.${key}</span>
									</p>
								</c:otherwise>
							...
							<p class="title">
								<em>${ty[0]}${fn:length(ty) > 1 ? ']' : (fn:startsWith(ty[0], '[') ? ']' : '')}</em>
