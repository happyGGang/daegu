...
$(document).ready(function() {
	...
	$('input#imsi_v_4').datepicker({
		maxDate: $('input#imsi_v_5').val(),
		onClose: function(selectedDate) {
			$('input#imsi_v_5').datepicker('option', 'minDate', selectedDate);
		}
	});
	$('input#imsi_v_5').datepicker({
		minDate: $('input#imsi_v_4').val(),
		onClose: function(selectedDate) {
			$('input#imsi_v_4').datepicker('option', 'maxDate', selectedDate);
		}
	});
});
</script>
		...
		<tbody>
          ...
          <c:otherwise>
			...
			<tr>
				<th>문화포털 게시일</th>
				<td colspan="3">
					<form:input type="text" path="imsi_v_4" cssClass="text ui-calendar"/> ~
					<form:input type="text" path="imsi_v_5" cssClass="text ui-calendar"/>
				</td>
			</tr>
