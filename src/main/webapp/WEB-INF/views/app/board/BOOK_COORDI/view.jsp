		...
		<div style="margin-top: -25px; background: #fffef9; border: 1.5px solid #f6f7e5; padding: 15px; border-radius: 10px;">
			<ul>
				<li style="margin-bottom: 10px;">
					<i>성별 : </i><span>${not authMBA ? '***' : board.imsi_v_1}</span>
				</li>
				<li style="margin-bottom: 10px;">
					<i>연락처 : </i><span>${not authMBA ? '***' : board.user_phone}</span>
				</li>
				<li style="margin-bottom: 10px;">
					<i>이메일 : </i><span>${not authMBA ? '***' : board.user_email}</span>
				</li>
			</ul>
		</div>
		<div class="bbs-view-body">
			...
			<dl>
				...
				<dd class="file">
					...
					<c:forEach var="i" varStatus="status" items="${j.boardFile}">
						<li><a href="${getContextPath}/board/boardFile/download/${j.manage_idx}/${i.board_idx}/${i.file_idx}.do"><i class="fa <boardTag:file_ext file_ext="${i.file_ext_name}"/>"></i><span>${i.org_file_name}</span></a></li>
					</c:forEach>
					</ul>
				</dd>
				</c:if>
