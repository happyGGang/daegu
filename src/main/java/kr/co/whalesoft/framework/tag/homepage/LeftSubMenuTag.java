...
public class LeftSubMenuTag extends BodyTagSupport {
					...
					if (menu.getMenu_level() >= 3) {
						if (StringUtils.isNotEmpty(link_url)) {
							...
							if (preMenuLevel < menu.getMenu_level()) {
								...
								li.setAttribute("class", "sub_mini_menu_list");
								liTag_lvl.get(menu.getMenu_level() - 1).addSubTag(li);
								ulTag.add(menu.getMenu_level() - 1, li);
							}
							HtmlTag div = new HtmlTag("div");
							div.setAttribute("id", "menu_" + menu.getMenu_idx());
							div.setAttribute("class", "sub_mini_menu_list_item");
							div.setContent("<a href=\"" + link_url + "\" " + targetStr + "><span>" + menu.getMenu_name() + "</span></a>");
							liTag_lvl.add(menu.getMenu_level(), div);
							ulTag.get(menu.getMenu_level() - 1).addSubTag(div);;
						}
					} else {
						if (StringUtils.isNotEmpty(link_url)) {
							if (menu.getMenu_type().equals("LINK_OUTER")) {
							...
							if (preMenuLevel < menu.getMenu_level()) {
								...
								ul.setAttribute("class", "sub_menu_list_box");
							...
							li.setAttribute("class", "sub_menu_list_item");
