...
public class UntactLockerSettingController extends BaseController {
	...
	public String index(Model model, UntactLockerSetting untactLockerSetting, HttpServletRequest request) throws AuthException {
		...
		model.addAttribute("untactLockerSettingList", untactLockerSettingService.getUntactLockerSettingList(getAsideHomepageId(request)));
	...
	public @ResponseBody JsonResponse save(UntactLockerSetting untactLockerSetting, BindingResult result, HttpServletRequest request) {
		...
		if (!result.hasErrors()) {
			untactLockerSettingService.modifyUntactLockerSetting(untactLockerSetting);
			res.setValid(true);
	...
	public @ResponseBody JsonResponse modifyAll(UntactLockerSetting untactLockerSetting, BindingResult result, HttpServletRequest request) {
		...
		if (!result.hasErrors()) {
			untactLockerSettingService.modifyUntactLockerSettingALL(untactLockerSetting);
			res.setValid(true);
		} else {
			res.setValid(false);
			res.setResult(result.getAllErrors());
	...
	public String bookSettingEdit(Model model, UntactBookSetting untactBookSetting, HttpServletRequest request) throws AuthException {
		...
		model.addAttribute("termsList", untactLockerSettingService.getUntactBookSettingTerms(untactBookSetting.getHomepage_id()));
	...
	public @ResponseBody JsonResponse bookSettingSave(UntactBookSetting untactBookSetting, BindingResult result, HttpServletRequest request) {
		...
		ValidationUtils.rejectIfZero(result, "reserable_count", "예약가능 권수를 입력하세요.");
		...
		if(untactBookSetting.getReservation_repeated_day() > 1) {
			if(untactLockerSettingService.checkUntactBookRoundCount(untactBookRound) > 1) {
				res.setValid(false);
		...
		if (!result.hasErrors()) {
			try {
				if(untactLockerSettingService.checkSetting(untactBookSetting) > 0) {
					untactLockerSettingService.mergeUntactBookSetting(untactBookSetting);
					res.setValid(true);
					res.setMessage("수정 되었습니다.");
				} else {
					if(untactLockerSettingService.createUntactBookRound(untactBookSetting) > 0) {
						untactLockerSettingService.mergeUntactBookSetting(untactBookSetting);
						res.setValid(true);
