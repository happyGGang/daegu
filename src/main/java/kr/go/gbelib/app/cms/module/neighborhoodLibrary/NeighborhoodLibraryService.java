...
public class NeighborhoodLibraryService extends BaseService {
	...
	public @ResponseBody JsonResponse updateNeighborhoodLibrary(NeighborhoodLibrary neighborhoodLibrary, HttpServletRequest request) {
		...
		if("2".equals(neighborhoodLibrary.getReserve_status())) { // 2: 예약확정
			...
			status3.setReserve_bundle_idx(neighborhoodLibrary.getReserve_bundle_idx()); //번들 idx가 같으면 같은 건수 이므로 같은 번들 idx가 있는지 검색
			NeighborhoodLibrary sameReserveOne = sameNeighborhoodLibraryForUser(status3);

			if(sameReserveOne != null) { // 한건에 두건이면 번들 idx로 두개 같이 업데이트
				status3_update.setReserve_bundle_idx(sameReserveOne.getReserve_bundle_idx());
			}else {
				status3_update.setReserve_bundle_idx(neighborhoodLibrary.getReserve_idx());
			}
...
//				}
	...
	public Map<String, Object> checkReserveLocker(NeighborhoodLibrary neighborhoodLibrary) {
		Map<String, Object> result = new HashMap<String, Object>();
		NeighborhoodLibrary resultReserve = new NeighborhoodLibrary();
		resultReserve = dao.checkReserveLocker(neighborhoodLibrary);
