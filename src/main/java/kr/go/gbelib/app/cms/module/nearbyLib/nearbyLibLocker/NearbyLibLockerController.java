 ...
 */
@Controller
@RequestMapping(value = {"/cms/module/nearbyLibLocker"})
public class NearbyLibLockerController extends BaseController {

private final String basePath = "/cms/module/nearbyLib/nearbyLibLocker/";
	
	
	@Autowired
	private NearbyLibLockerService service;
	...
	public String index(Model model, NearbyLibLocker nearbyLibLocker, HttpServletRequest request) {
		//개별 사물함 불러오기
		List<NearbyLibLocker> lockerOneList = service.getNeighborhoodLibraryLockerEachOneList(nearbyLibLocker);
		NearbyLibLocker lockerOne = service.getNeighborhoodLibraryLockerOne(nearbyLibLocker);
		
		//
		NearbyLib neigborhoodLibrary = new NearbyLib();
		neigborhoodLibrary.setDevice_idx(nearbyLibLocker.getDevice_idx());
		List<NearbyLib> neighborhoodLibraryList = neighborhoodLibraryService.getNeighborhoodLibraryList(neigborhoodLibrary);
	...
	public String edit(Model model, NearbyLibLocker nearbyLibLocker, HttpServletRequest request) {
	...
	public String use_edit(Model model, NearbyLibLocker nearbyLibLocker, HttpServletRequest request) {
		NearbyLibDevice neighborhoodLibraryDevice = new NearbyLibDevice();
		neighborhoodLibraryDevice.setDevice_idx(nearbyLibLocker.getDevice_idx());
	...
	public String detail(Model model, NearbyLibLocker nearbyLibLocker, HttpServletRequest request) {
		
		if(nearbyLibLocker.getDevice_idx() > 0) {
			nearbyLibLocker = service.getNeighborhoodLibraryLockerOne(nearbyLibLocker);
		}
		model.addAttribute("locker", nearbyLibLocker);
	...
	public @ResponseBody JsonResponse save(Model model, NearbyLibLocker nearbyLibLocker, BindingResult result, HttpServletRequest request) {
		JsonResponse res = new JsonResponse(request);
		
		if(nearbyLibLocker.getEditMode().equals("ADD") || nearbyLibLocker.getEditMode().equals("MODIFY")) {
...
//			ValidationUtils.rejectIfEmpty(result, "add_row_no", "모니터 열에 존재하는 사물함 갯수를 입력하세요.");
		...
		if (!result.hasErrors()) {
			if (nearbyLibLocker.getEditMode().equals("ADD")) {
				...
				service.addNeighborhoodLibraryLocker(nearbyLibLocker);
				res.setValid(true);
				res.setMessage("등록되었습니다.");
			} else if (nearbyLibLocker.getEditMode().equals("MODIFY")) {
//				service.modifyNeighborhoodLibraryLocker(neighborhoodLibraryLocker);
				res.setValid(true);
				res.setMessage("수정되었습니다.");
			}else if(nearbyLibLocker.getEditMode().equals("DELETE")) {
//				service.deleteNeighborhoodLibraryLocker(neighborhoodLibraryLocker);
	...
	public @ResponseBody JsonResponse locker_edit(Model model, NearbyLibLocker nearbyLibLocker, BindingResult result, HttpServletRequest request) {
		JsonResponse res = new JsonResponse(request);
		...
		if (!result.hasErrors()) {			
			service.modifyNeighborhoodLibraryLocker(nearbyLibLocker);
			res.setValid(true);
