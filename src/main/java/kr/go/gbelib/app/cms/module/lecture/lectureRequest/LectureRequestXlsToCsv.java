...
public class LectureRequestXlsToCsv {

	public LectureRequestXlsToCsv(List<LectureRequest> lectureRequestList, String fileName, Homepage homepage, HttpServletRequest request, HttpServletResponse response) {
		try {
			OutputStream out = response.getOutputStream();
			WritableWorkbook workbook = Workbook.createWorkbook(out);
			workbook = new LectureRequestWorkbook().workbookForm(workbook, lectureRequestList, homepage, request, response);
			response.reset();
