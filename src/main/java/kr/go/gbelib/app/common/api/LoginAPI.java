...
//		param.put("api_key", "79724C6D73152DC1035B16B6198665D34A640D5D11E8ACD60083FA80FE417E58");
	...
	public static Object rfidLogin(Member member) throws Exception {
		...
		param.put("option", 4);
		param.put("api_key", "79724C6D73152DC1035B16B6198665D34A640D5D11E8ACD60083FA80FE417E58");
		param.put("workno", member.getLogin_id().toUpperCase());
		...
		if (StringUtils.equals(resultInfo, "SUCCESS")) {
			...
			if (userMap != null && !userMap.isEmpty()) {
				...
				String address = String.valueOf(userMap.get("H_ADDR1"));
				if (StringUtils.isNotEmpty(address) && !StringUtils.equals(address, "null")) {
					member.setAddress1(address);
					member.setAddress(address);
				}

				String handphone = String.valueOf(userMap.get("HANDPHONE"));
				if (StringUtils.isNotEmpty(handphone) && !StringUtils.equals(handphone, "null")) {
					member.setCell_phone(handphone);
					try {
						String[] handphone_arr = String.valueOf(userMap.get("HANDPHONE")).split("-");
						if (String.valueOf(userMap.get("HANDPHONE")) != null && !String.valueOf(userMap.get("HANDPHONE")).equals("")) {
							member.setCell_phone(String.valueOf(userMap.get("HANDPHONE")));
						}
						if (handphone_arr[0] != null && !handphone_arr[0].equals("null") && !handphone_arr[0].equals("")) {
							member.setCell_phone1(handphone_arr[0]);
						} else {
				...
				String phone = String.valueOf(userMap.get("H_PHONE"));
				if (StringUtils.isNotEmpty(phone) && !StringUtils.equals(phone, "null")) {
					member.setPhone(phone);
					try {
						String[] phone_arr = phone.split("-");
				...
				String email = String.valueOf(userMap.get("E_MAIL"));
				if (StringUtils.isNotEmpty(email) && !StringUtils.equals(email, "null")) {
					member.setEmail(email);
					try {
						String[] email_arr = email.split("@");
	...
	public static Object barcodeLogin(Member member) throws Exception {
		...
		param.put("option", 3);
		param.put("user_no", member.getLogin_id());
		param.put("manage_code", member.getManage_code());
		param.put("api_key", "79724C6D73152DC1035B16B6198665D34A640D5D11E8ACD60083FA80FE417E58");
