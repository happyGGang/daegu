...
public class PrivateLoginAPI {

	protected final static Logger log = LoggerFactory.getLogger(PrivateLoginAPI.class);
	...
	public static Object login(Member member) throws Exception {
		...
		Map<String, Object> loginMap = CommonAPI.sendPrivateKCMS("userlogin", param);
		...
		if (StringUtils.equals(resultInfo, "SUCCESS")) {
			...
			if (userMap != null && !userMap.isEmpty()) {
				...
				try {
					Map<String, Object> libSettingInfoView = PrivateLibSearchAPI.getLibSettingInfoView(member.getUser_manage_code(), null, null, null, null, null);
					@SuppressWarnings("unchecked")
					...
					log.error("사립도서관 getlibcode error");
				}
				...
				if (StringUtils.equals(member.getMember_pw(), "test")) {
					List<Map<String, Object>> maps = PrivateMemberAPI.checkDupUser("1", member);
					if (maps != null) {
						userInfo = maps.get(0);
						userInfoResult = "SUCCESS";
					}
				} else {
					userInfo = PrivateMemberAPI.getUserInfo(member.getMember_id(), member.getMember_pw());
				...
				if (StringUtils.equals(userInfoResult, "SUCCESS")) {
					if (StringUtils.equals(member.getMember_pw(), "test")) {
						...
						memberInfo = PrivateLibSearchAPI.getListData(userInfo, "USER_DATA").get(0);
					}
